...
export class TransactionHistoryListener {
  ...
  async handleLlmCompletion(event: LlmCompletionEvent) {
    ...
    const llmPrice = await prisma.llmModelPricing.findUnique({
    ...
    if (!llmPrice) throw new Error('Model pricing not found ' + model);

    const { price, pricingMethod } = llmPrice;
  ...
  calcPrice_deepseek(price: DeepseekPricing, usage: LLMResponse['usage']) {
