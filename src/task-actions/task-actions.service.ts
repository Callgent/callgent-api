...
import { BotletApiActionDto } from '../botlet-api-actions/dto/botlet-api-action.dto';
...
export class TaskActionsService {
  ...
  async execute(
    ...
    const req = reqAdaptor.toDs(rawReq, true, reqEndpoint);
  ...
  protected async _invocationFlow(
    ...
    const actions = await prisma.botletApiAction.findMany({ where, take });
  ...
  protected async _routing(req: RequestPack, actions: BotletApiActionDto[]) {
  ...
  @Transactional()
  async callout(action: BotletApiActionDto, req: RequestPack) {
